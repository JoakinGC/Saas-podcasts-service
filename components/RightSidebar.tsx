      ...
      <SignedIn>
        <Link href={`/profile/${user?.id}`} className="flex gap-3 pb-12">
          ...
          <div className="flex w-full items-center justify-between">
            <h1 className="text-16 truncate font-semibold text-black-1 dark:text-white-1">{user?.firstName} {user?.lastName}</h1>
          ...
          {topPodcasters?.slice(0,4).map((podcaster) => (
            ...
            >
              <figure className="flex items-center gap-2">
                ...
                />
                <h2 className="text-14 font-semibold text-black-1 dark:text-white-1">{podcaster.name}</h2>
